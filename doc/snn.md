...
- end - 0x8 (4 bytes): the size of each neuron state (8 + 4 * numMC bytes)
- end - 0xC (4 bytes): the type of neurons:
  - 0x0: FP32 LIF neurons. This is currently the only defined type.
...
At the start of each timestep, the input accumulation is done, the input cleared (we're using a current-based LIF model). No firing and reset is done at this point.
